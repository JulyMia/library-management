...
- 重构后通过 `_validate_borrow_request()` 和 `_create_borrow_record()` 把借书过程拆成对象获取 -> 规则校验 -> 执行借阅三个阶段，主流程更接近业务语义。
...
- 这样虽然辅助函数本身仍有一定复杂度，但复杂度被封装到了更合适的局部位置，公开接口更短、更利于阅读和后续测试。
...
- 这种重构属于典型的提取模板方法，它不会改变业务结果，但能显著降低分支复杂度和重复率，是本次实验最容易量化体现收益的改动之一。

## 综合结论
- 三个关键改动分别作用在业务主流程、复杂字段更新和 CLI 菜单分发三个层面。
- 它们共同体现出的重构思想是：把主流程保留为简短骨架，把变化点和细节处理下沉到小函数中。
- 这种方式虽然让代码文件变长了一些，但明显提升了阅读顺序、修改定位效率和后续测试可切入性。
